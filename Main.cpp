...
ULeaderboard* g_SteamLeaderboards = NULL;
...
AMain::AMain()
{
	...
	RunningSpeed = 650.f;
	SprintingSpeed = 1400.f;

	bShiftKeyDown = false;
	bLMBDown = false;


	ScoreboardWidget = CreateDefaultSubobject<UUserWidget>(TEXT("Scoreboard"));
...
void AMain::BeginPlay()
{
	...
	if (bRet)
	{
		g_SteamLeaderboards = NewObject<ULeaderboard>();
	}
...
void AMain::Tick(float DeltaTime)
{
	...
	PointTotal = CallPointTotalFromBlueprint();
	int UploadScore(PointTotal);

	FHitResult OutHit;
	FVector Start = GetActorLocation();

	FVector DownwardVector = GetActorUpVector();
	FVector End = ((DownwardVector * (-150.f)) + Start);
	FCollisionObjectQueryParams CollisionObjectParams;
	FCollisionQueryParams CollisionParams;
	CollisionObjectParams.AddObjectTypesToQuery(ECC_Pawn);
	//CollisionParams = ECC_Pawn;

	//DrawDebugLine(GetWorld(), Start, End, FColor::Green, false, 2.f);

	AMain::bCurrentLineTrace = GetWorld()->LineTraceTestByObjectType(Start, End, CollisionObjectParams, CollisionParams);

	AEnemy* Enemy = Cast<AEnemy>(MainPlayerController);
	if (Enemy)
	{
		if (bCurrentLineTrace == true)
		{

			if (Enemy->SmokeParticles)
			{
				const USkeletalMeshSocket* EnemySocket = GetMesh()->GetSocketByName("rootSocket");
				if (EnemySocket)
				{
					FVector SocketLocation = EnemySocket->GetSocketLocation(GetMesh());
					UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), Enemy->SmokeParticles, SocketLocation, FRotator(0.f), false);
				}
			}
...
void AMain::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("MenuSelect", IE_Pressed, this, &AMain::MenuSelect);
...
void AMain::IncrementSpoiledEggs(int32 Amount)
{
	SpoiledEggs += Amount;
}

void AMain::IncrementEggs(int32 Amount)
{
	Eggs += Amount;
}

void AMain::IncrementKills(int32 Amount)
{
	EnemyKills += Amount;
}
...
void AMain::IncrementArmor(float Amount)
{
...
void AMain::DecrementArmor(float Amount)
{
	if (Armor - Amount <= 0.f)
	{
		Armor -= Amount;
...
void AMain::DeathMovementStatus()
{
...
void AMain::Die()
{
	...
	AnimInstance->Montage_Play(CombatMontage, 2.f, ReturnValueType, 4.f, true);
	AnimInstance->Montage_Pause(CombatMontage);

	GetWorld()->GetTimerManager().SetTimer(TimerHandle, this, &AMain::DeathMovementStatus, 3.f, false);
}
	...
	if (!bAttacking && MovementStatus != EMovementStatus::EMS_Dead)
	{
		...
		if (AnimInstance && CombatMontage)
		{
			...
			case 0:
				...
				AnimInstance->Montage_JumpToSection(FName("Anim_Attack"), CombatMontage);
				break;
			case 1:
...
void AMain::SaveGame()
{
	...
	SaveObject->CharacterStats.Eggs = Eggs;
	SaveObject->CharacterStats.EnemyKills = EnemyKills;
...
void AMain::LoadGame(bool LoadPosition)
{
	...
	if (LoadObject)
	{
		...
		Eggs = LoadObject->CharacterStats.Eggs;
		EnemyKills = LoadObject->CharacterStats.EnemyKills;
...
int AMain::CallPointTotalFromBlueprint()
{
	FOutputDeviceNull ar;
	int total = 0;
	const FString command = FString::Printf(TEXT("CalculatePointTotal 0"));
	if (ScoreboardWidget)
	{
		total = ScoreboardWidget->CallFunctionByNameWithArguments(*command, ar, NULL, true);
	}
...
void AMain::DelayAndDisableInput(){}